...
interface ProjectQuickSettingsProps {
  project: Project;
...
const ProjectQuickSettings: React.FC<ProjectQuickSettingsProps> = ({
  project,
  currentUser,
  isOpen,
  onClose,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const updatedProject = { ...project, ...settings };
  ...
  const leaveProject = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/projects/${project.projectId}/leave`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        ...
        onClose();
        // 프로젝트 목록 새로고침을 위해 페이지 리로드하거나 적절한 콜백 호출
        window.location.reload();
      } else {
        const errorData = await response.json();
        throw new Error(errorData.error || '프로젝트 나가기에 실패했습니다.');
      }
    } catch (error) {
      addToast({
        type: 'error',
        title: '나가기 실패',
        message: error instanceof Error ? error.message : '프로젝트 나가기 중 오류가 발생했습니다.',
        duration: 5000
      });
    } finally {
      setLoading(false);
                  ...
                  <div className="mt-6 pt-6 border-t border-border">
                    <div className="bg-destructive/10 rounded-lg p-4">
                      <div className="flex items-center space-x-3">
                        ...
                        <button
                          onClick={() => {
                            if (confirm('정말로 이 프로젝트에서 나가시겠습니까? 이 작업은 되돌릴 수 없습니다.')) {
                              leaveProject();
                            }
                          }}
                          disabled={loading}
...
export default ProjectQuickSettings;